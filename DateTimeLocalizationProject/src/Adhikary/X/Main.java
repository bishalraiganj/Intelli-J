...
public class Main {
    ...
    public static void main(String... args)
    {
        ...
        Employee joe = new Employee("Joe","en_AU","Australia/Eucla");
        ...
        int days = 10;
        Map<LocalDate,List<ZonedDateTime>> map = schedule(joe,jane,days);
        ...
        for(LocalDate ldt : map.keySet())
        {
            ...
            {
                System.out.println("\t" +
                        jane.getDateInfo(zdt,dtf) + " <--> " + joe.getDateInfo(zdt.withZoneSameInstant(joe.zone),dtf));
            }
        }
    ...
    private static Map<LocalDate, List<ZonedDateTime>> schedule(Employee first,Employee second,int days)
    {
        ...
        return startingDate.datesUntil(startingDate.plusDays(days + 1))
                .map((e)->e.atStartOfDay(first.zone()))
                .flatMap((e)-> IntStream.range(0,24).mapToObj((e1)->e.withHour(e1)))
                .filter(rules)
                .map((e)->e.withZoneSameInstant(second.zone))
                .filter(rules)
                .collect(Collectors.groupingBy((ZonedDateTime e)->e.toLocalDate(),()->new TreeMap<>(),Collectors.toList()));
